...
crop_ROI = [
    ...
    1500,
    2500,
]  # Determined manually because the tiling is messing up the registration

I1_cropped = I1[
    crop_ROI[1] : (crop_ROI[1] + crop_ROI[3]), crop_ROI[0] : (crop_ROI[0] + crop_ROI[2])
]
correctedI2_cropped = corrected_I2[
    crop_ROI[1] : (crop_ROI[1] + crop_ROI[3]), crop_ROI[0] : (crop_ROI[0] + crop_ROI[2])
]
...
alpha_synuclein = sk.io.imread(
    "../data/Dataset 1/round 001/AW GVB AM1c-s11 010426_A01_w1.tif"
)
...
SK1delta_corr = SK1delta_corr[
    crop_ROI[1] : (crop_ROI[1] + crop_ROI[3]), crop_ROI[0] : (crop_ROI[0] + crop_ROI[2])
]

## Find synuclein positive cells
alpha_synuclein_disp = sk.exposure.rescale_intensity(
    alpha_synuclein, in_range=(0, 2500), out_range=(0.0, 1.0)
)
...
overlay = sk.segmentation.mark_boundaries(
    sk.exposure.equalize_hist(chmp2b),
    mask_spots_filt,
    mode="thick",
    color=(1.0, 0, 1.0),
)
